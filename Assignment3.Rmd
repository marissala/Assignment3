...
```{r}
pacman::p_load(ggplot2, rethinking, gridExtra)
...
# Use non-scaled values for the models with single predictors (to be able to interpret better)
...
V_model <- map(
  alist(
    ...
    a ~ dnorm(17.5, 7.2), #VerbalIQ prior, whats the expected Verbal IQ when ADOS is 0?
...
nV_model <- map(
  alist(
    NonVerbalIQ.s ~ dnorm(mu,sigma), #likelihood
    mu <- a + b*ADOS.s, #linear model
    a ~ dnorm(26.8, 5.5), #NonVerbalIQ prior, whats the expected Verbal IQ when ADOS is 0?
    b ~ dnorm(0, 5), #expected change for a unit of 1, 5 means am agnostic
    sigma ~ dunif(0, 50) #sigma prior
...
S_model <- map(
  alist(
    SocialIQ.s ~ dnorm(mu,sigma), #likelihood
    mu <- a + b*ADOS.s, #linear model
    a ~ dnorm(77.2, 10.4), #SocialIQ prior, whats the expected Verbal IQ when ADOS is 0?
    b ~ dnorm(0, 5), #expected change for a unit of 1, 5 means am agnostic
    sigma ~ dunif(0, 50) #sigma prior
...
model.all.s <- map(
  alist(
    ...
    a ~ dnorm(0, 10), #VerbalIQ prior, whats the expected Verbal IQ when ADOS is 0?
    bNon ~ dnorm(0, 10), #expected change for a unit of 1, 5 means am agnostic
    bVer ~ dnorm(0, 10),
    bSoc ~ dnorm(0, 10),
    sigma ~ dunif(0, 50) #sigma prior
  ) ,
...
# What if I try to see a difference in Social IQ for the 2 groups?
S_model <- map(
  alist(
    SocialIQ.s ~ dnorm(mu,sigma), #likelihood
    mu <- a + b*ADOS.s, #linear model
